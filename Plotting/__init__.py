...
def main():
	...
	games_won = raw_data[raw_data["GameWon"] == True]
	games_lost = raw_data[raw_data["GameWon"] == False]
	...
	columns = games_won.columns
	for col in columns:
		if col == "Race":
			continue

		bplot = sns.catplot(y=col, x='Race', data=games_lost, kind="box", height=10, palette="muted", aspect=3)
		# bplot.set_xticklabels(rotation=45, labels=races)
		file_path = "{0}/Race_GameLost_Box/{1}.png"
		print(col)
